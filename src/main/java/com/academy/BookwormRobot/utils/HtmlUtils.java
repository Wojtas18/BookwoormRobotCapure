...
public class HtmlUtils {
	 ...
	 * @return
	 */
	public static String getInnerContentFromSelkar(String element, String url, int elementNo) {
		String content = null;
		try {
			UserAgent userAgent = new UserAgent();
			userAgent.visit(url);
			content = userAgent.doc.findEach(element).getElement(elementNo).getText();
		} catch (JauntException e) {
			logger.error(e.getMessage());
		}
